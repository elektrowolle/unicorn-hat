...
#define UNICORNHAT_TARGET_FREQ    WS2811_TARGET_FREQ
#define UNICORNHAT_GPIO_PIN       18
#define UNICORNHAT_DMA            5

#define UNICORNHAT_WIDTH          8
#define UNICORNHAT_HEIGHT         8
#define UNICORNHAT_LED_COUNT      (UNICORNHAT_WIDTH * UNICORNHAT_HEIGHT)
static class unicornHat {
public:
    ws2811_t ledstring =
    {
      .freq = UNICORNHAT_TARGET_FREQ,
      .dmanum = UNICORNHAT_DMA,
      .channel =
        {
          [0] =
            {
              .gpionum    = UNICORNHAT_GPIO_PIN,
              .count      = UNICORNHAT_LED_COUNT,
              .invert     = 0,
              .brightness = 55,
            }
        }
    ...
    void clearLEDBuffer(void) {
      int i;
      for (i = 0; i < UNICORNHAT_LED_COUNT; i++) {
...
*/
    ...
    void render(){
      for(int i = 0; i < 64; i++) {
        setPixelColorRGB(getPixelPosition(i % 8, i / 8),
                         pixel[i * 3 + 0],
                         pixel[i * 3 + 1],
                         pixel[i * 3 + 2]);
      }
...
// try:
/*
 * UNICORN_HAT.initialise();
 * unsigned char pixel = UNICORN_HAT.getPixel();
 * //funny pixel stuff;
 * UNICORN_HAT.render();
 */
